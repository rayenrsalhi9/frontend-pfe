...
export class EmailSmtpSettingService {
  ...
  ): Observable<EmailSMTPSetting | CommonError> {
    const url = `email-smtp-setting`;
    return this.httpClient
      .post<EmailSMTPSetting>(url, setting)
      .pipe(catchError(this.commonHttpErrorService.handleError));
  }

  updateEmailSMTPSetting(
    setting: EmailSMTPSetting,
  ): Observable<EmailSMTPSetting | CommonError> {
    const url = `email-smtp-setting/${setting.id}`;
    return this.httpClient
      .put<EmailSMTPSetting>(url, setting)
      .pipe(catchError(this.commonHttpErrorService.handleError));
  }

  deleteEmailSMTPSetting(
    id: string,
  ): Observable<EmailSMTPSetting | CommonError> {
    const url = `email-smtp-setting/${id}`;
    return this.httpClient
      .delete<EmailSMTPSetting>(url)
      .pipe(catchError(this.commonHttpErrorService.handleError));
  }
}
