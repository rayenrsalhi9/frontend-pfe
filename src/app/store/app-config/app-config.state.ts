...
export class AppConfigState {
  ...
  @Action(UpdateConfig)
  updateConfig(
    { getState, patchState }: StateContext<AppConfig>,
  ...
  changeSideNavCollapse(
    { getState, patchState }: StateContext<AppConfig>,
    { sideNavCollapse }: UpdateSideNavCollapse,
  ) {
    const state = getState();
    patchState({
      ...state,
  ...
  changeMobileNavCollapse(
    { getState, patchState }: StateContext<AppConfig>,
    { mobileNavCollapse }: UpdateMobileNavCollapse,
  ) {
    const state = getState();
    patchState({
      ...state,
